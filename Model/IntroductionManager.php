...
use N1c0\LessonBundle\Event\IntroductionPersistEvent;
...
abstract class IntroductionManager implements IntroductionManagerInterface
{
     ...
     * @return Introduction
     */
    public function createIntroduction(LessonInterface $lesson)
    {
        $class = $this->getClass();
        $introduction = new $class;

        $introduction->setLesson($lesson);
     ...
     * @throws InvalidIntroductionException when the introduction does not have a lesson.
     */
    public function saveIntroduction(IntroductionInterface $introduction)
    {
        if (null === $introduction->getLesson()) {
            throw new InvalidIntroductionException('The introduction must have a lesson');
        }
